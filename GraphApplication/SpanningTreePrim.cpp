...
CSpanningTree SpanningTreePrim(CGraph & graph)
{
	...
	//Cua amb prioritat d’arestes que connecten vèrtex de l’arbre amb altres que encara no són de l’arbre.
	...
	while (!queue.empty() && vertex_afegits < graph.GetNVertices())
	{
		...
		// Si l'origen o el destí ja esta a l'arbre agafem la destinacio
		CVertex* pV;
